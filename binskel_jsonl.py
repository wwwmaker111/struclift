...
def collate_binskel_stage3_batch(
    ...
    *,
    ...
    则按提案 §7.3.2 构造 ``slot_mask``；否则退化为全 1（与旧 jsonl 兼容）。
    ...
    base: Dict[str, Any] = collate_binskel_stage2_batch(samples)
    B = len(samples)
    ...
    for gi, (si, sl, st) in enumerate(trimmed):
        ...
        if (
            ...
            or not isinstance(bb_lists, list)
            or len(pos_list) != len(bb_lists)
        ):
            continue
...
def collate_binskel_stage3_with_skip(
    ...
    *,
    ...
    return collate_binskel_stage3_batch(
        good, pad_token_id=pad_token_id, max_sft_seq_len=max_sft_seq_len,
    )
...
def collate_binskel_stage4_batch(
    ...
    *,
    ...
    base = collate_binskel_stage3_batch(
        samples, pad_token_id=pad_token_id, max_sft_seq_len=max_sft_seq_len,
    )
    B = len(samples)
    C = num_patterns
...
class Stage2CurriculumDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.datasets: List[BinSkelJsonlDataset] = []
...
def make_binskel_stage4_dataloader(
    ...
    *,
    ...
    ``max_sft_seq_len``：同 :func:`make_binskel_stage3_dataloader`。
    """
    if isinstance(jsonl_path, (str, Path)):
        paths: List[Path] = [Path(jsonl_path)]
    else:
