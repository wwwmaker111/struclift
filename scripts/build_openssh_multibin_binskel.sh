#!/usr/bin/env bash
# OpenSSH multibin：对每个 multibin_* 内 ELF 跑 build_binskel，合并为 5 条 jsonl。
#
# 用法:
#   bash scripts/build_openssh_multibin_binskel.sh <openssh_src_dir> <out_dir> [structlift_workdir]
#
set -euo pipefail
...
run_tier() {
  ...
  [ -d "$bindir" ] || { echo "缺少 $bindir，先运行 scripts/compile_openssh_multibin.sh"; exit 1; }
  [ -d "$ir" ] || { echo "缺少 $ir"; exit 1; }

  local part="$OUT/.openssh_multibin_parts_${tag}"
  rm -rf "$part"
  mkdir -p "$part"
  ...
  echo ""
  echo "===== openssh multibin $opt_arg (${#bins[@]} 个) ====="
  local i=0
  for elf in "${bins[@]}"; do
    [ -f "$elf" ] || continue
    i=$((i+1))
  ...
  local outj
  if [ "$tag" = "os" ]; then
    outj="$OUT/binskel_openssh_os_multibin.jsonl"
  else
    outj="$OUT/binskel_openssh_${tag}_multibin.jsonl"
  fi
  : > "$outj"
...
echo ""
echo "完成: binskel_openssh_o{0..3}_multibin.jsonl + binskel_openssh_os_multibin.jsonl"
