#!/usr/bin/env bash
#
# 按 scripts/binskel_alignment_batch_stats.py 默认「9 项目」顺序批量重导 binskel jsonl：
#   zlib → curl → sqlite → busybox → openssl → openssh → libxml2 → ffmpeg → coreutils
#
# 【与 45 档命名是否一致】
#   - 本脚本对 **coreutils** 调用的是 build_coreutils_binskel.sh → 产出 binskel_coreutils_o0.jsonl（单 ELF，无 _multibin 后缀）。
#   - scripts/build_binskel_9proj_5opt.sh / augment_all_45_to_full_sft.py 期望的是
#     binskel_coreutils_o0_multibin.jsonl（compile_coreutils_multibin + build_coreutils_multibin_binskel）。
#   - 若你的训练/样例路径含 multibin_o*，请改用 build_binskel_9proj_5opt.sh，勿用本脚本生成 coreutils。
#
# 与论文 BinSkel-Train 表格相比：多 openssh、ffmpeg、coreutils。
#
...
#   export LIBXML2_DIR=...
#   export FFMPEG_DIR=...
#   # coreutils：需先有 coreutils_o0..o3、ir_o0..ir_o3（见 scripts/compile_coreutils.sh）
#   export COREUTILS_DIR=~/struclift_wsl/datasets/coreutils/coreutils-9.4
#
#   bash scripts/rebuild_binskel_all_9.sh
#
# 已从 zlib/curl 跑完、只想续跑后面几项时：
#   export START_AT=sqlite   # 或 busybox / openssl / openssh / libxml2 / ffmpeg / coreutils
#   bash scripts/rebuild_binskel_all_9.sh
#
...
# 从某一步骤起跑，避免重复 zlib/curl。合法值：zlib curl sqlite busybox openssl openssh libxml2 ffmpeg coreutils
START_AT="${START_AT:-zlib}"
_valid_steps=(zlib curl sqlite busybox openssl openssh libxml2 ffmpeg coreutils)
_ok=
...
should_run_step coreutils  && run_one "coreutils"  "build_coreutils_binskel.sh"  "COREUTILS_DIR"
