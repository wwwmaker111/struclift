...
def _maybe_torch_compile_stage2(model: StrucLift, *, rank: int) -> None:
    """可选：对冻结 Stage2 路径上的三大模块做 torch.compile，缩短 encoder/bwd 耗时。

    需 PyTorch 2.x；图形状多变时用 ``dynamic=True``。首轮若干 step 可能因编译较慢。
    设环境变量 ``STRUCLIFT_TORCH_COMPILE_STAGE2=1`` 启用。
    """
    if os.environ.get("STRUCLIFT_TORCH_COMPILE_STAGE2", "").strip() != "1":
        return
    if not hasattr(torch, "compile"):
        if rank == 0:
            logger.warning(
                "STRUCLIFT_TORCH_COMPILE_STAGE2=1 但当前 PyTorch 无 torch.compile，已跳过",
            )
        return
    try:
        model.binary_encoder = torch.compile(
            model.binary_encoder, dynamic=True, mode="default",
        )
        model.source_encoder = torch.compile(
            model.source_encoder, dynamic=True, mode="default",
        )
        model.alignment_module = torch.compile(
            model.alignment_module, dynamic=True, mode="default",
        )
        if rank == 0:
            logger.info(
                "[Stage2] torch.compile(dynamic=True) 已作用于 binary_encoder / "
                "source_encoder / alignment_module；首轮若干 step 可能较慢（编译热身）",
            )
    except Exception as e:
        if rank == 0:
            logger.warning("torch.compile 失败（已跳过）: %s", e)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="每多少 epoch 并入下一优化档（默认读 config s2_curriculum_epochs_per_stage）；与 --curriculum-adaptive 二选一",
    )
    ap.add_argument(
        "--curriculum-adaptive",
        action="store_true",
        help="按 val total + 每档 min/max epoch / patience 自动进档（须 --curriculum 与 --val-jsonl-per-curriculum-stage）；切档不再用 curriculum_epochs_per_stage",
    )
    ap.add_argument(
        "--curriculum-stage-min-epochs",
        type=int,
        default=None,
        help="自适应进档：每档至少训满若干 epoch（默认 config s2_curriculum_stage_min_epochs）",
    )
    ap.add_argument(
        "--curriculum-stage-max-epochs",
        type=int,
        default=None,
        help="自适应进档：每档至多若干 epoch 后强制进档（默认 config s2_curriculum_stage_max_epochs）",
    )
    ap.add_argument(
        "--curriculum-stage-patience",
        type=int,
        default=None,
        help="自适应进档：val total 连续无改善次数（默认 config s2_curriculum_stage_patience）",
    )
    ap.add_argument(
        "--curriculum-stage-min-delta",
        type=float,
        default=None,
        help="自适应进档：视为有提升所需的 val_total 降幅下限（默认 config s2_curriculum_stage_min_delta）",
    )
    ap.add_argument(
        ...
        default=None,
        help=(
            "（仅与 --curriculum-adaptive + --resume 同用）显式指定本次起步的 curriculum stage "
            "（0..N-1）；用于从旧的非自适应 ckpt 切换到自适应、且不愿回到 stage 0 的场景。"
            "会保留 ckpt 的模型/优化器/epoch/best_loss，但**覆盖** ckpt 内 (若有) 的 "
            "curriculum_adaptive 状态：本档计数清零、best_val_stage 重置。"
        ),
    )
    ap.add_argument(
        "--no-node-hard-neg",
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "可选：Stage2 encoder cache 根目录（由 scripts/precompute_stage2_encoder_cache.py "
            "预生成）。每个 jsonl 在该目录下用 ``<jsonl_stem>/manifest.json`` 子目录定位 cache。"
            "命中后训练跳过 binary/source encoder forward 与 SubgraphPatternClassifier，"
            "stage4 单 epoch 量级可压到原 ~1/2。前提：cache 用与本次 --init-from / --resume 同一份 stage1 权重生成。"
        ),
    )
    ap.add_argument(
        "--encoder-cache-shard-lru",
        type=int,
        default=4,
        help="每个 DataLoader worker 内 shard LRU 大小（每 shard ~50MB；num_workers>0 时各占一份内存）",
    )
    ap.add_argument(
        "--no-tf32",
        action="store_true",
    ...
    if args.curriculum_adaptive and args.val_jsonl_per_curriculum_stage is None:
        ap.error(
            "--curriculum-adaptive 需要 --val-jsonl-per-curriculum-stage（与课程档数一致的多份 val）",
        )
    if args.initial_curriculum_stage is not None and not args.curriculum_adaptive:
        ap.error("--initial-curriculum-stage 需要 --curriculum-adaptive")
    if args.initial_curriculum_stage is not None and args.resume is None:
        ap.error("--initial-curriculum-stage 需要 --resume（用于保留模型/优化器；课程档位由该参数覆盖）")
    ...
    if args.curriculum_adaptive:
        cfg.training.s2_curriculum_adaptive = True
    if args.curriculum_stage_min_epochs is not None:
        cfg.training.s2_curriculum_stage_min_epochs = int(args.curriculum_stage_min_epochs)
    if args.curriculum_stage_max_epochs is not None:
        cfg.training.s2_curriculum_stage_max_epochs = int(args.curriculum_stage_max_epochs)
    if args.curriculum_stage_patience is not None:
        cfg.training.s2_curriculum_stage_patience = int(args.curriculum_stage_patience)
    if args.curriculum_stage_min_delta is not None:
        cfg.training.s2_curriculum_stage_min_delta = float(args.curriculum_stage_min_delta)
    ...
    if args.curriculum:
        ...
        cache_dir_resolved: Path | None = (
            args.encoder_cache_dir.expanduser().resolve()
            if args.encoder_cache_dir is not None else None
        )
        curriculum_ds = Stage2CurriculumDataset(
            paths_by_opt,
            max_samples=args.max_samples,
            skip_invalid=skip_invalid,
            encoder_cache_dir=cache_dir_resolved,
            encoder_cache_shard_lru=int(args.encoder_cache_shard_lru),
        )
        ...
        if not use_ddp or rank == 0:
            logger.info(
                ...
                f"  (encoder_cache_dir={cache_dir_resolved})" if cache_dir_resolved else "",
            )
    ...
    if args.init_from is not None and args.resume is None:
        ...
        if missing:
            logger.info("未载入键（示例最多 5 个）: %s", missing[:5])
        if unexpected:
    ...
    if args.initial_curriculum_stage is not None:
        assert curriculum_ds is not None
        k = int(args.initial_curriculum_stage)
        mx = int(curriculum_ds.max_stage())
        if k < 0 or k > mx:
            logger.error(
                "--initial-curriculum-stage=%d 越界（合法范围 0..%d）", k, mx,
            )
            sys.exit(1)
        trainer._curriculum_adaptive = True
        trainer._ad_stage = k
        trainer._ad_epochs_in_stage = 0
        trainer._ad_no_improve = 0
        trainer._ad_best_val_stage = float("inf")
        resume_info["best_loss"] = float("inf")
        if rank == 0:
            logger.info(
                "[Stage2] 已覆盖 curriculum_adaptive 状态：起始 stage=%d/%d, 本档计数清零, "
                "best 基线重置为 +inf（用于切换自适应口径）",
                k,
                mx,
            )

    _maybe_torch_compile_stage2(model, rank=rank)
    ...
    # 算「开训那一刻 trainer 真正会用的样本数」：
    #   - 自适应 curriculum 且通过 --initial-curriculum-stage 覆盖了 stage，loader 现在还
    #     停在初始 stage 0，trainer.train() 进入时会 set_stage 并 rebuild loader；
    #     这里按目标 stage 给 curriculum_ds 算一遍 len，避免日志里误打成 stage 0 的样本数。
    _n_log = len(loader.dataset)
    if curriculum_ds is not None and getattr(trainer, "_curriculum_adaptive", False):
        _eff_stage = int(getattr(trainer, "_ad_stage", 0))
        _eff_stage = max(0, min(_eff_stage, curriculum_ds.max_stage()))
        _prev_stage = curriculum_ds.get_stage()
        curriculum_ds.set_stage(_eff_stage)
        _n_log = len(curriculum_ds)
        curriculum_ds.set_stage(_prev_stage)
    logger.info(
